...
const BotBuilderFlow = ({ closeBuilder, editingBot }) => {
  ...
  // DELETE NODE FUNCTIE (Verbonden aan het ✕ kruisje)
  ...
  useEffect(() => {
    ...
    if (editingBot && editingBot.settings.ui_layout) {
        ...
        // Fix voor overlap: Posities zijn nu goed verdeeld (naast elkaar in plaats van op elkaar)
        setNodes([
  ...
  const handleSaveAndCompile = async () => {
    try {
        ...
        if (!configNode) return alert("Compile Error: Missing 'Main Configuration' block.");
        if (!whitelistNode) return alert("Compile Error: Missing 'Asset Whitelist' block.");

        const symbolsList = whitelistNode.data.pairs.split(',').map(s => s.trim()).filter(s => s.length > 0);
        if (symbolsList.length === 0) return alert("Compile Error: Whitelist must contain at least one pair.");
        ...
        if (entryNode) {
             ...
             const takeProfits = tpEdges.map(e => {
             ...
             const stopLosses = slEdges.map(e => {
                 const n = nodes.find(nd => nd.id === e.source);
                 return { type: n.data.triggerType, value: n.data.triggerValue, close_amount_type: n.data.closeType, close_amount_value: n.data.closeValue };
             });
        ...
        if (!payload.settings.entry_node) {
            alert("Warning: No Entry (LONG) logic connected. Engine will ignore this algorithm.");
        }

        if (editingBot) {
            await apiClient.put(`/api/bots/${editingBot.id}`, { name: payload.name, settings: payload.settings });
            alert("Algorithm Configuration Updated.");
        } else {
            await apiClient.post('/api/bots/', payload);
            alert("Algorithm Successfully Compiled & Deployed.");
        }
        closeBuilder();

    } catch (err) {
        console.error(err);
        alert("Compile Error: " + (err.response?.data?.detail || err.message));
    }
  };
      ...
      <div className="w-72 bg-[#181a20] border-r border-[#2b3139] flex flex-col z-10 shadow-lg">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            <div className="space-y-3">
                ...
                <div className="p-3 bg-[#0b0e11] border border-[#8b5cf6]/50 rounded text-[11px] text-[#8b5cf6] font-bold cursor-grab hover:bg-[#8b5cf6]/10 transition-colors uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'botConfig')} draggable>
                  Main Configuration
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#d946ef]/50 rounded text-[11px] text-[#d946ef] font-bold cursor-grab hover:bg-[#d946ef]/10 transition-colors uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'whitelist')} draggable>
                  Asset Whitelist
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#fcd535]/50 rounded text-[11px] text-[#fcd535] font-bold cursor-grab hover:bg-[#fcd535]/10 transition-colors uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'backtest')} draggable>
                  Backtest Engine
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#0ea5e9]/50 rounded text-[11px] text-[#0ea5e9] font-bold cursor-grab hover:bg-[#0ea5e9]/10 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'apiKey')} draggable>
                  Exchange Routing
                </div>
            </div>
            ...
            <div className="space-y-3">
                ...
                <div className="p-3 bg-[#0b0e11] border border-[#2b3139] rounded text-[11px] text-[#eaecef] font-bold cursor-grab hover:bg-[#2b3139]/50 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'indicator')} draggable>
                  Technical Indicator
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#3b4149] rounded text-[11px] text-[#eaecef] font-bold cursor-grab hover:bg-[#3b4149]/50 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'condition')} draggable>
                  Data Condition
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#2ea043]/50 rounded text-[11px] text-[#2ea043] font-bold cursor-grab hover:bg-[#2ea043]/10 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'logic')} draggable>
            ...
            <div className="space-y-3">
                ...
                <div className="p-3 bg-[#0b0e11] border border-[#2ebd85]/50 rounded text-[11px] text-[#2ebd85] font-bold cursor-grab hover:bg-[#2ebd85]/10 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'takeProfit')} draggable>
                  Take Profit (Target)
                </div>
                <div className="p-3 bg-[#0b0e11] border border-[#f6465d]/50 rounded text-[11px] text-[#f6465d] font-bold cursor-grab hover:bg-[#f6465d]/10 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'stopLoss')} draggable>
                  Stop Loss (Risk)
                </div>
            </div>
            ...
            <div className="space-y-3">
                ...
                <div className="p-3 bg-[#0b0e11] border border-[#eaecef]/20 rounded text-[11px] text-[#eaecef] font-bold cursor-grab hover:bg-[#eaecef]/10 transition-colors shadow-sm uppercase tracking-wider" onDragStart={(event) => onDragStart(event, 'action')} draggable>
                  Action Routing
                </div>
            </div>
